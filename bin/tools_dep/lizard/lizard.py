...
###############################################################################
TOTAL_CYCLOMATIC_COMPLEXITY, TOTAL_FILES_NUM = 0, 0
###############################################################################
...
###############################################################################
            reader.context.current_function.condition_lines_list.add(reader.context.current_line)
###############################################################################
...
###############################################################################
    global TOTAL_CYCLOMATIC_COMPLEXITY
    global TOTAL_FILES_NUM
###############################################################################
    for file_info in module_infos:
        if file_info:
###############################################################################
            TOTAL_CYCLOMATIC_COMPLEXITY += float(file_info.average_cyclomatic_complexity)
            TOTAL_FILES_NUM += 1
###############################################################################
...
###############################################################################
    def clang_warning_format(self):
		#路径->方法名->方法长名->开始行-结束行->方法总行数->圈复杂度
        return ("{f.filename}->{f.name}->{f.long_name}->{f.start_line}-{f.end_line}->{f.nloc}->{f.cyclomatic_complexity}->{f.condition_lines_list}")
...
###############################################################################
    if TOTAL_FILES_NUM != 0:
        print(str(TOTAL_CYCLOMATIC_COMPLEXITY)+":"+str(TOTAL_FILES_NUM))
###############################################################################
    return count
